...
class Controller_paciente:
    ...
    def inserir_Paciente(self) -> Paciente:
        ...
        if self.verifica_existencia_cliente(oracle, cpf):
            ...
            dt_nasc = input("Data de Nascimento (Novo): ")
            sexo = input("Sexo (Novo): ")
            celular = input("Celular (Novo): ")
            email = input("Email (Novo): ")
            # Insere e persiste o novo cliente
            oracle.write(f"insert into paciente values ('{nome}', '{dt_nasc}', '{sexo}', '{cpf}', '{celular}', '{email}')")
